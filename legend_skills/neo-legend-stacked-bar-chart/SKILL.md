---
name: neo-legend-stacked-bar-chart
description: >-
  Maintain and tune the Neo Legend `stacked_bar_chart` renderer. Use when changing this chart's visual style, layout, data mapping, theme behavior, sample image, tests, or generated report output in Neo_Legend_Project.
  维护和调优 Neo Legend `stacked_bar_chart`（堆叠柱状图）渲染器。当需要在 Neo_Legend_Project 中更改此图表的视觉风格、布局、数据映射、主题行为、示例图片、测试或生成的报告输出时使用。
---
...
- Legend type: `stacked_bar_chart`（图例类型：堆叠柱状图）
- Renderer class: `StackedBarChartSkill`（渲染器类）
- Source file: `src/neo_legend/business_report_charts.py`（源文件）
- Default style: `wall_street_stack`（默认样式：华尔街堆叠风格）
- Default size: `1400x900`（默认尺寸）
...
| --- | --- | --- | --- | --- | --- |
| `wall_street_stack` | 华尔街堆叠风格 | Rounded stacked bar chart for executive reports. | 面向行政报告的圆角堆叠柱状图。 | `G:/echaet/stacked_bar_chart__wall_street_stack.png` | |
| `portfolio_stack` | 投资组合堆叠 | Portfolio allocation stacked bars with premium theme. | 高级主题的投资组合分配堆叠柱状图。 | `G:/echaet/stacked_bar_chart__portfolio_stack.png` | |
...
1. Treat this Skill as the product-level spec for `stacked_bar_chart`.（将此技能作为 `stacked_bar_chart` 的产品级规范。）
